...
func (p *Parser) parseClassDeclaration() (d.Stmt, error) {
	...
	var superclass *d.VariableExpr
	if p.match(d.LESS) {
		_, err := p.consume(d.IDENTIFIER, "Expect superclass name")
		if err != nil {
			return nil, err
		}
...
func (p *Parser) parsePrimary() (d.Expr, error) {
	...
	if p.match(d.SUPER) {
		keyword := p.previous()
		_, err := p.consume(d.DOT, "Expect '.' after 'super'.")
		if err != nil {
			return nil, err
		}

		method, err := p.consume(d.IDENTIFIER, "Expect superclass method name")
		if err != nil {
			return nil, err
		}
		return d.SuperExpr{
